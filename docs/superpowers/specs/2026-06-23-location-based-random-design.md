...
- **주변 식당 목록을 화면에 보여주지 않는다.** 목록은 추첨용 내부 데이터로만
  쓰고 결과 1곳만 표시한다. 따라서 페이지네이션·더보기 UI도 만들지 않는다.

## 제품 방향(맥락)

장기적으로 로그인 도입 후 한줄평, 내 단골/블랙리스트 같은 개인화 기능을
추가할 예정이다. 이번 전환은 그 로드맵을 막지 않는 선에서 진행한다.

리스트 화면·한줄평·북마크는 한 묶음으로 다음 단계에서 함께 설계한다(식당을
선택해 평·북마크를 다는 흐름이 자연스럽게 리스트를 요구하기 때문). 이번 단계는
`findNearbyRestaurants`가 식당 **목록**을 반환하도록만 해두어, 다음 단계에서 그
목록을 화면에 뿌리고 페이지네이션을 붙이기만 하면 되도록 구조를 열어둔다.
식당 식별자(카카오 검색 결과의 `id` 등)도 그 단계에서 활용하면 되므로 지금
별도로 보존하지 않는다.
...
5. 결과 카드 + 지도 모달 표시(목록은 화면에 노출하지 않음).
